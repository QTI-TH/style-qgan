...
\begin{abstract}
  We propose an alternative quantum generator architecture in the context of generative adversarial learning, and we assess this model for the generation of
  Monte Carlo events. We validate
  the methodology by implementing our architecture on artificial data generated from
  known underlying distributions. Then, we model Monte Carlo simulated
  events for particle physics processes at the Large Hadron Collider. We benchmark its performance and demand high accuracy results. The new quantum generator architecture leads to an improvement on the state-of-the-art implementations using shallow quantum neural networks. Finally, we successfully deploy our model into two different quantum hardware architectures, namely, superconducting qubit and trapped ion technologies.
\end{abstract}
