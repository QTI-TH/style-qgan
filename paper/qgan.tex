...
In the last row of Figure~\ref{fig:3dgaus} we compare the one dimensional
...
model has been trained on $10^4$ samples. See Table~\ref{table:summary_lhc} for
the more details. In this case, we perform a linear pre-processing of the data
...
selected in particular the machine {\tt ibm\_santiago}, a 7-qubit machine
...
not. {\color{red} J.: need to update this to the machines we've
  actually used for the plots, and add the number of shots we
  use. Marco, you may have a look at it.}

We present in Figure~\ref{fig:ibm} examples of samples that have been
generated using {\tt ibm\_santiago} machine on IBM-Q. We use a 3-qubit
...
generated by the style-qGAN on the {\tt ibm\_santiago} quantum
...
\begin{figure*}
  ...
  model on {\tt ibm\_santiago} (top row) trained with $10^4$ samples and
...
\begin{figure*}
  %\includegraphics[width=0.25\textwidth]{plots/hardware/ionQ/s-t_REAL_1000_100.pdf}%
  %\includegraphics[width=0.25\textwidth]{plots/hardware/ionQ/t-y_REAL_1000_100.pdf}%
  %\includegraphics[width=0.25\textwidth]{plots/hardware/ionQ/y-s_REAL_1000_100.pdf}
