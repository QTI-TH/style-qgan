...
"We find agreement between the reference and generated eigenvalues to the better of $\lesssim 10\%$ for style-qGAN setups with equal and more than 3 latent dimensions. With $D_{latent}< 3$ we observe significant deviations of factors $\mathcal{O}(10)$ while for $D_{latent}\geq 3$ no further significant improvement is seen. The same holds for increasing the number of layers in the style-qGAN model. This suggests that he number of latent dimensions introduced through the entangling gates $U_{ent}$ are the key hyperparameters once the number of layers allows a sufficient complexity to model a single dimension. However, training success also depends on the equilibration of the GAN through optimisation. In practice this might make a larger than minimal set of layers and parameters a better choice and we suggest to use the recipe of this paragraph as rule of thumb only."}
