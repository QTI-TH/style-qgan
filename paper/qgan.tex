...
\begin{figure*}
  ...
  $pp \rightarrow t\bar{t}$ production using the style-qGAN generator model on IBM Santiago (top row) trained
  with $10^4$ samples and the corresponding ratio to the reference underlying
  prior MC distribution (bottom row). The style-qGAN generator model deployed in this quantum hardware still manages to capture the correlations and provides reasonably good samples when compared to the reference
  distribution.}
\end{figure*}
