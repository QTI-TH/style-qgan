...
\begin{figure*}
  ...
  provides acceptable samples when compared to the reference distribution. Note that we choose a grey background for the plots in the bottom row to highlight when the reference and generated samples are identical.}
\end{figure*}
...
In the second row of Figure~\ref{fig:ttbar} we compare the one dimensional
cumulative projections of samples generated by the style-qGAN model with the
reference input distribution function (red histogram) for $10^5$ samples. Also
...
\begin{figure*}
  ...
  with $10^4$ samples and the marginal sample distribution (middle row)
  for each dimension. The bottom row shows the corresponding ratio to the reference underlying
  prior MC distribution.  The style-qGAN generator model learns the correlations and
  provides acceptable samples when compared to the reference distribution. Note that we choose a grey background for the plots in the bottom row to highlight when the reference and generated samples are identical.
}
\end{figure*}
...
Figure~\ref{fig:ttbar} because of the noise \commentCBP{and reduced capacity of the quantum generator}, but still quite
reasonable. The
...
\begin{figure*}
  ...
  the marginal sample distribution (middle row)
  for each dimension. Corresponding ratio to the reference underlying prior MC
  distribution (bottom row). Note that we choose a grey background for the plots in the bottom row to highlight when the reference and generated samples are identical. }
\end{figure*}
...
\begin{figure*}
  ...
  with $10^4$ samples and the marginal samples distribution (middle row) for each dimension compared to the the reference underlying prior MC
   distribution. Corresponding ratio to the generated samples on actual quantum hardware (bottom row).
   \commentCBP{In the bottom row, do we want to plot noise simulation/generated or reference/noise simulation ? Anyhow, please Julien could you change the title to show the full word "simulation" instead of "simu"?}}
\end{figure*}
