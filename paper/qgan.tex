...
decided to consider a hybrid classical-quantum system, where the generator model
has a quantum representation through variational quantum circuits, while the
discriminator is a classical neural-network model. This choice is motivated by
the practical implications of a generative model that can be deployed in a real
quantum device. In fact, the idea of using a quantum device for the generation
of samples by delegating the complicated aspects of density modelling and
sampling to a hardware architecture is very attractive.

The implementation of generative adversarial models requires the definition of a
generator and discriminator models in a training procedure where the generator
learns how to extract fake samples with realistic

Talk about quantum-quantum vs classical-quantum

Figure~\ref{fig:scheme} we show schematically the components discussed above.
