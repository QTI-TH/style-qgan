...
\item Maybe small discussion about Fig 3 to highlight why we include it -- typical GAN training convergence that shows that archicture does what it needs to do \commentJB{done on lines 274-277, should be enough?}
\item Add Fig 5 discription of gray and white to make it clear (and so people know to look on computer if need be) \commentJB{done}
\item Keep working on this $10^4$ vs $10^5$ training versus generated vs reference \commentJB{this should be clear now I think}
\item Green line for Fig 7 sanity check with different pull of $10^4$ reference samples. Anthony (+ Carlos for aid if need be) \commentJB{I think this we forget about it, so consider this point done}
\end{itemize}
\fi
