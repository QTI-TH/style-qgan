...
\commentAF{Good point. If we want to write this we need to explain how and in what way we improve. We could add this in the following sections explicitly mentioning the improvements other qGANs. Alternatively we could add a sentence here, but then it might be better situated at the end of section I. }
...
this level we study the covariance matrices defined by the reference and the generated samples. The summed eigenvalues of the reference and generated covariance matrices give a means to estimate the similarity between the learned underlying correlations. We find agreement between the reference and generated eigenvalues to the better of $\lesssim10\%$ for style-qGAN setups with equal and more than 3 latent dimensions. This observation was largely independent on the number of layers suggesting that the number of latent dimensions introduced through the entangling gates $U_{ent}$ are the key hyperparameters. \commentCBP{Anthony, could you explain this last reasoning? I do not understand why that is the case.}
\commentAF{Carlos, my thinking was that the the success of learning the covariances depends almost entirely on the latent dimensions and not the number of layers. I wanted to somehow incorporate our observation of the spaghetti plots. But perhaps the current sentence does not capture this. Here's another try:\\
"We find agreement between the reference and generated eigenvalues to the better of $\lesssim 10\%$ for style-qGAN setups with equal and more than 3 latent dimensions. Recall that the latent variables are introduced in every gate of the circuit, including the entangling ones $U_{ent}$.  With $D_{latent}< 3$ we observe significant deviations of factors $\mathcal{O}(10)$ while for $D_{latent}\geq 3$ no further significant improvement is seen. The same holds for increasing the number of layers in the style-qGAN model. This suggests that he number of latent dimensions introduced are the key hyperparameters once the number of layers allows a sufficient complexity to model a single dimension. 
However, training success also depends on the equilibration of the GAN through optimisation. In practice this might make a larger than minimal set of layers and parameters a better choice and we suggest to use the recipe of this paragraph as rule of thumb only."}
