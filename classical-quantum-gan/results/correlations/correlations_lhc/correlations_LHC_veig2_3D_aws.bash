...
dat1 = np.loadtxt(f"./results/target_{nqubits}_{latent_dim}_{layers}.aws.3dhist",usecols=(3,)); 
...
dat1 = np.loadtxt(f"./results/qc_{nqubits}_{latent_dim}_{layers}.aws.3dhist",usecols=(3,)); 
dat1.shape = (nbin,nbin,nbin)
...
dat1 = np.loadtxt(f"./results/qc_{nqubits}_{latent_dim}_{layers}.aws2.3dhist",usecols=(3,)); 
dat1.shape = (nbin,nbin,nbin)
...
for b1 in range(0,nbin):
	for b2 in range(0,nbin):
		for b3 in range(0,nbin):
			ma1[b1]+=mmat[b1,b2,b3]
			ma2[b1]+=mmat[b2,b1,b3]
			ma3[b1]+=mmat[b2,b3,b1]
...
kl_divergence1a = np.around(0.5* (entropy(mt1+offset, ma1+offset) + entropy(ma1+offset, mt1+offset)) , 3) 
kl_divergence2a = np.around(0.5* (entropy(mt2+offset, ma2+offset) + entropy(ma2+offset, mt2+offset)) , 3) 
kl_divergence3a = np.around(0.5* (entropy(mt3+offset, ma3+offset) + entropy(ma3+offset, mt3+offset)) , 3) 
print(kl_divergence1a,kl_divergence2a,kl_divergence3a)

print(kl_divergence1a+kl_divergence2a+kl_divergence3a)



outf3 = open("measure.aws.veig2_3D_${lay}", "a")
